...
`vc +detail` 只能拿到 `note_id` 和 `minute_token`，不直接返回纪要文档 token 与妙记产物内容。要获取实际产物，需根据用户诉求继续调用 `note +detail` 或 `minutes +detail`，**并沿用第 1 步同一个 `--as`**（完整规则见 [lark-shared](../../lark-shared/GUIDE.md) 的「身份延续」）：
